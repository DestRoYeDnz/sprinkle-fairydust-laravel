...
class CalculatorSettings
{
    ...
    public static function rules(): array
    {
        return [
            ...
            'form.organizerPhone' => ['nullable', 'string', 'max:48'],
            ...
            'form.guestCount' => ['nullable', 'numeric', 'min:0', 'max:500'],
            'form.packageName' => ['nullable', 'string', 'max:120'],
            'form.servicesRequested' => ['nullable', 'string', 'max:500'],
            'form.travelArea' => ['nullable', 'string', 'max:255'],
            'form.venueType' => ['nullable', Rule::in(['indoor', 'outdoor', 'mixed', 'unsure', ''])],
            'form.heardAbout' => ['nullable', 'string', 'max:120'],
            ...
            'form.includeDiscount' => ['required', 'boolean'],
            'form.discountName' => ['nullable', 'string', 'max:120'],
            'form.discountDescription' => ['nullable', 'string', 'max:255'],
            'form.discountAmount' => ['required', 'numeric', 'min:0'],
    ...
    public static function normalize(array $input): array
    {
        ...
        return [
            ...
            'form' => [
                ...
                'organizerPhone' => (string) data_get($settings, 'form.organizerPhone', ''),
                ...
                'guestCount' => (float) data_get($settings, 'form.guestCount', 0),
                'packageName' => (string) data_get($settings, 'form.packageName', ''),
                'servicesRequested' => (string) data_get($settings, 'form.servicesRequested', ''),
                'travelArea' => (string) data_get($settings, 'form.travelArea', ''),
                'venueType' => (string) data_get($settings, 'form.venueType', ''),
                'heardAbout' => (string) data_get($settings, 'form.heardAbout', ''),
                'eventAddress' => (string) data_get($settings, 'form.eventAddress', ''),
                'notes' => (string) data_get($settings, 'form.notes', ''),
                'termsAccepted' => (bool) data_get($settings, 'form.termsAccepted', false),
                ...
                'customAddOns' => collect(data_get($settings, 'form.customAddOns', []))
                    ...
                    ->values()
                    ->all(),
                'includeDiscount' => (bool) data_get($settings, 'form.includeDiscount', false),
                'discountName' => mb_substr(trim((string) data_get($settings, 'form.discountName', 'Discount')), 0, 120),
                'discountDescription' => mb_substr(trim((string) data_get($settings, 'form.discountDescription', '')), 0, 255),
                'discountAmount' => max(0.0, (float) data_get($settings, 'form.discountAmount', 20)),
