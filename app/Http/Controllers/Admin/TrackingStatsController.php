...
class TrackingStatsController extends Controller
{
    ...
    public function index(): JsonResponse
    {
        ...
        $trackingByAnonymousId = PageView::query()
            ...
            ->selectRaw('SUM(CASE WHEN event_type = ? THEN 1 ELSE 0 END) as page_views', ['view'])
            ->selectRaw('SUM(CASE WHEN event_type = ? AND page_key = ? THEN 1 ELSE 0 END) as gallery_views', ['view', 'gallery'])
            ->selectRaw('SUM(CASE WHEN event_type = ? AND page_key = ? THEN 1 ELSE 0 END) as design_views', ['view', 'designs'])
            ->selectRaw('SUM(CASE WHEN event_type = ? THEN COALESCE(duration_seconds, 0) ELSE 0 END) as total_time_seconds', ['engagement'])
